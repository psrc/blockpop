...
Runs PopulationSim in place against the project's existing folders: the
``popsim_configs/`` directory (settings.yaml + controls.csv), the project
``data`` directory (seed tables, crosswalk, and marginals), and the project
...
This is a GUI-invoked utility (not a pypyr pipeline step): PopulationSim
is launched from the Population Synthesis tab, not the control-totals
pipeline.
...
def run(pipeline: Pipeline) -> Path:

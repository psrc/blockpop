...
from blockpop.util.census_api import (
    CensusAPI,
...
def totals_data(pipeline):
    pipeline.create_directory(path=str(pipeline._get_intermediate_dir() / 'acs_data'))
    census_api = CensusAPI(pipeline.CENSUS_KEY)
    print('Downloading ACS totals')
    variables_dict = build_totals_variables_dict(pipeline.acs_config_dir)
    try:
        dfs = []
    ...
    pipeline.save_table('acs_data/acs_totals', df, fmt='csv')
    return df
...
def get_selected_groups(pipeline):
    ...
    if yaml_path.exists():
        return build_aggregated_variables_by_group(
            pipeline.acs_config_dir, 'marginals_expressions.csv', yaml_path
        )
    return build_variables_by_group(pipeline.acs_config_dir, 'marginals_expressions.csv')
...
def acs_data(pipeline,totals_df):
    ...
    for group_name, variables_dict in groups.items():
        ...
        pipeline.save_table(f'acs_data/{group_name}', group_df, fmt='csv')
        check_totals(totals_df, group_name, group_df, pipeline.acs_config_dir / 'marginals_expressions.csv')
...
def evaluate_acs_expressions(pipeline):
    ...
    totals = pipeline.get_table('acs_data/acs_totals').set_index('geoid').drop(columns='name')

    for group_name, group_expr_rows in expr_rows.groupby('group'):
        print(f'Evaluating expressions for group: {group_name}')
        group_df = pipeline.get_table(f'acs_data/{group_name}')
        ...
        pipeline.save_table(f'acs_data/{group_name}', df, fmt='csv')

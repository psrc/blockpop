...
def compute_hhpop_rates(pipeline):
    ...
    county_df = pipeline.get_table('dec_data/county_data')
...
def compute_hhpop_age(pipeline, rates):
    ...
    total_pop_age = pipeline.get_table('acs_data/total_pop_age')
...
def normalize_hhpop_age(pipeline, df):
    totals = pipeline.get_table('acs_data/acs_totals').set_index('geoid')['hhpop']
    result = df[['geoid', 'name']].copy()
...
def run_step(context):
    ...
    pipeline.save_table('acs_data/hhpop_age', hhpop_age, fmt='csv')

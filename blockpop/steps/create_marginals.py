...
# blockpop/configs/group_totals.csv.
...
def _load_group_totals(pipeline):
    ...
    configs_root = Path(str(resources.files('blockpop.configs')))
...
def _load_special_control_totals(pipeline):
    ...
    """
    mapping = {}
    for config_dir in (pipeline.acs_config_dir, pipeline.dec_config_dir):
...
def _build_block_lookup(pipeline):
    ...
    path = Path(pipeline.data_dir) / 'puma_block_lookup.csv'
    if not path.exists():
    ...
    for group, targets in grouped.items():
        ...
        for t in targets:
            if t not in group_df.columns:
                raise RuntimeError(f"Column '{t}' missing from acs_data/{group}")
...
def run_step(context):
    ...
    controls_path = pipeline.base_dir / 'popsim_configs' / 'controls.csv'
    ...
    # Source tables.
    dec_totals_block = pipeline.get_table('dec_data/dec_totals')
    dec_totals_block['geoid'] = dec_totals_block['geoid'].astype(np.int64)
    tenure_block = pipeline.get_table('dec_data/tenure')
    tenure_block['geoid'] = tenure_block['geoid'].astype(np.int64)
    acs_totals_tract = pipeline.get_table('acs_data/acs_totals')
    ...
    pipeline.create_directory(path=str(data_dir))
